...
export default function Edit({ salesOrder, referenceData }: Props) {
    const { data, setData, put, processing, errors, reset } = useForm({
        so_number: salesOrder.so_number,
        so_date: salesOrder.so_date,
        customer_id: salesOrder.customer_id.toString(),
        currency_id: salesOrder.currency_id,
        order_type_id: salesOrder.order_type_id.toString(),
        status_type_id: salesOrder.status_type_id.toString(),
        remarks: salesOrder.remarks || '',
        pph_id: salesOrder.pph_id?.toString() || '',
        vat_id: salesOrder.vat_id?.toString() || '',
    ...
    const [details, setDetails] = useState<SalesOrderDetail[]>(
        salesOrder.details?.map(detail => ({
            ...
            discount_type: detail.discount_type as 'percentage' | 'amount' | '',
    ...
    const updateDetail = (index: number, field: keyof SalesOrderDetail, value: any) => {
        ...
        if (['qty', 'unit_price', 'discount_type', 'discount_value'].includes(field)) {
            ...
            if (detail.discount_type && detail.discount_type !== 'none' && discountValue > 0) {
                ...
                } else if (detail.discount_type === 'amount') {
                ...
                <div className="max-w-7xl mx-auto sm:px-6 lg:px-8">
                    <AppBreadcrumb
                        items={[
                            ...
                            { label: salesOrder.so_number, href: `/sales-orders/${salesOrder.id}` },
                    ...
                    <Card>
                        <CardHeader>
                            <CardTitle className="text-xl sm:text-2xl">Edit Sales Order #{salesOrder.so_number}</CardTitle>
                            <CardDescription>
                                Update the sales order details and line items
                            </CardDescription>
                        </CardHeader>
                        <CardContent>
                            <form onSubmit={submit} className="space-y-8">
                                ...
                                <Card>
                                    ...
                                    <CardContent className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select value={data.order_type_id.toString()} onValueChange={(value) => setData('order_type_id', value)}>
                                                <SelectTrigger>
                                        ...
                                        <div className="space-y-2">
                                            ...
                                            <Select value={data.status_type_id.toString()} onValueChange={(value) => setData('status_type_id', value)}>
                                                <SelectTrigger>
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                                    <div className="space-y-2">
                                        ...
                                        <Select value={data.pph_id?.toString() || 'none'} onValueChange={(value) => setData('pph_id', value === 'none' ? '' : value)}>
                                            <SelectTrigger>
                                    ...
                                    <div className="space-y-2">
                                        ...
                                        <Select value={data.vat_id?.toString() || 'none'} onValueChange={(value) => setData('vat_id', value === 'none' ? '' : value)}>
                                            <SelectTrigger>
                                    ...
                                    <CardHeader>
                                        <div className="flex flex-col space-y-4 sm:flex-row sm:justify-between sm:items-center sm:space-y-0">
                                            <div>
                                                ...
                                                <CardDescription>Add and manage line items for the sales order</CardDescription>
                                            </div>
                                                    ...
                                                    {details.map((detail, index) => (
                                                        <TableRow key={index}>
                                                            ...
                                                            <TableCell className="min-w-36">
                                                                <Select
                                                                    value={detail.discount_type || 'none'}
                                                                    onValueChange={(value) => updateDetail(index, 'discount_type', value === 'none' ? '' : value)}
                                                                >
                                                                    <SelectTrigger className="w-full">
                                                            ...
                                                            <TableCell className="font-medium min-w-24">
                                                                <div className="text-sm">
                                                                    {(typeof detail.total_price === 'number' ? detail.total_price : parseFloat(detail.total_price) || 0).toFixed(2)}
                                                                </div>
                                                            </TableCell>
